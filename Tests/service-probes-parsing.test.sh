...
if has_connected_wifi_device_status ":wifi:connected:Home WiFi" "Home WiFi"; then
    echo "connected Wi-Fi status without a device was accepted" >&2
    exit 1
fi
...
if is_passwd_row "osso:x:1000:1000:Alessio:/home/osso:not-a-shell"; then
    echo "passwd row with invalid shell path was accepted" >&2
    exit 1
fi
...
if read_os_release_value BLANK_VALUE $'BLANK_VALUE=   \nID=noctalia-test' >/dev/null; then
    echo "blank os-release value was accepted" >&2
    exit 1
fi

if read_os_release_value BLANK_QUOTED_VALUE $'BLANK_QUOTED_VALUE="   "\nID=noctalia-test' >/dev/null; then
    echo "blank quoted os-release value was accepted" >&2
    exit 1
fi
...
assert_equal "$(list_quickshell_ipc_calls "spawn quickshell ipc -p $canonical_repo call launcher toggle-extra")" "" "malformed IPC function extraction failed"
...
if has_quickshell_launch_path "# exec quickshell -p $canonical_repo" "$canonical_repo"; then
    echo "commented wrapper launch path was accepted" >&2
    exit 1
fi
...
if has_stale_launch_path "# quickshell -p /home/osso/Repos/noctalia-shell" "/home/osso/Repos/noctalia-shell"; then
    echo "commented stale launch path was detected" >&2
    exit 1
fi
